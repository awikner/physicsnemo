...
- **Active branch:** `ai-rossby` — the core fork; PRs usually target this, not
  upstream `main`.
...
| **Polaris** (ALCF) | largest A100 pool; **best measured multi-node** | 560× 4-A100, 2 Slingshot NICs/node. PBS. No repo/data staged yet; `/eagle` is full. See [`hpc/polaris.md`](hpc/polaris.md). |
...
- **Polaris multi-node:** always pass `--cpu-bind depth -d 8` to `mpiexec`. Omitting
  it silently costs **9.1×** on inter-node NCCL bandwidth (4.08 vs 36.93 GB/s).
  Skip the `conda/*-aws-nccl-*` modules; see [`hpc/polaris.md`](hpc/polaris.md).
- **Delta multi-node:** compute nodes have **no InfiniBand** (Slingshot `hsn0`);
  load `aws-ofi-nccl/1.14.2` or NCCL falls back to TCP sockets (4.5× slower).
