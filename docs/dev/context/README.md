...
| [rsi-b40-correction-campaign](rsi-b40-correction-campaign.md) | **Corrected batch-40 series (2026-09-15):** A0 matched upstream's TOTAL loss at epoch 17 while its day-10 RMSE was 2.3x worse -- the deficit confined to the two highest-sigma window slots, which carry <30% of the weight (the staircase spends sigma^-1.1 of EDM-equivalent attention). New rungs `a0hn`/`a2lhn` with a high-noise loss boost; the A5 eps-family SDE fixed (`eps_mode=gamma2`, eps ~ Gamma^2) and shown to be a real spread knob; the FIRST genuine calibration metrics (`nrmse`/`ssr`/`crps`/rank -- the old spread-vs-rmse comparison mixed normalized and physical units); and `muon_momentum` was never reaching the optimizer, so every batch-40 chain ran at 0.95 |
